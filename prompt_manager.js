...
-🚨强制去重检查:记录前必须扫描表8所有行,检查语义是否重复(如"喜欢山竹"="喜欢吃山竹")
-🚨严禁分类错误:职业必须归入"基础信息",严禁放入"重要时刻"！宠物/饮食同理！
-操作:当发现新的用户相关信息时,先判断类型,再检查是否已存在同类型记录及内容去重.
-示例:用户提到自己的生日.
...
- 物品追踪:仅记录具有唯一性、剧情关键性或特殊纪念意义的道具(如:神器、钥匙、定情信物、重要礼物).严禁记录普通消耗品(食物/金钱)或环境杂物.物品必须唯一！若物品已在表中,无论它流转到哪里,都必须updateRow更新其[持有者]和[当前位置],严禁新建一行！
- 约定:仅记录双方明确达成共识的严肃承诺或誓言.必须包含{{user}}的主动确认.严禁记录单方面的命令、胁迫、日常行程安排或临时口头指令.
- 关于我(表8):记录{{user}}主动透露或对话中自然流露的关于自己的信息.内容分为七大类型:①基础信息(姓名/昵称/性别/年龄/生日/职业/居住地/星座),②个人特质(性格/技能/爱好),③偏好习惯(兴趣爱好/饮食偏好/沟通方式/禁忌事项),④人际关系(家人/朋友/宠物/其他关系),⑤重要时刻(重要经历/约定事项),⑥目标规划(短期目标3个月内/长期目标1年以上),⑦强调信息({{user}}明确要求记住的内容).
...
-⚠️这一行只能包含兴趣/饮食/沟通/禁忌,严禁出现年龄、职业、重要记忆等字段！
...
-⚠️宠物去重规则:每只宠物只能记录一次！添加宠物信息前必须检查是否已存在(如"比熊犬悠悠"和"宠物比熊犬"是同一信息)
...
⚠️操作规范(严格执行三步检查法):
📋步骤1:信息分类判断
-首先判断新信息属于哪个类型:基础信息/个人特质/偏好习惯/人际关系/重要时刻/目标规划/强调信息
-分类关键词参考:
  →基础信息:姓名/昵称/性别/年龄/生日/职业/工作/职位/居住地/星座
  →偏好习惯:喜欢/不喜欢/饮食/兴趣/禁忌/过敏
  →人际关系:家人/朋友/宠物/父母/孩子/配偶
...
-若信息不重复:
  →检查对应类型的行是否存在
  →若存在→updateRow追加到该类型行
  →若不存在→insertRow创建该类型行

-✅正确示例:
  用户说"我是产品经理" → 分类为"基础信息" → 检查表8是否已有"基础信息"行
    →若无: insertRow(8, {0: "基础信息", 1: "2024-01-20 10:00", 2: "职业:产品经理"})
    →若有: updateRow(8, 基础信息行索引, {1: "2024-01-20 10:00", 2: "原有内容;职业:产品经理"})

  用户说"我喜欢吃山竹" → 分类为"偏好习惯" → 检查是否已记录"山竹"
    →若已有"饮食:喜欢山竹" → 跳过(重复)
    →若无 → 检查"偏好习惯"行是否存在
      →若无: insertRow(8, {0: "偏好习惯", 1: "2024-01-20 10:05", 2: "饮食:喜欢山竹"})
      →若有: updateRow(8, 偏好习惯行索引, {2: "原有饮食内容,喜欢山竹"})
