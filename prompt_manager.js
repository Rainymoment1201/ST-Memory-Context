...
- 物品追踪:仅记录具有唯一性、剧情关键性或特殊纪念意义的道具(如:神器、钥匙、定情信物、重要礼物).严禁记录普通消耗品(食物/金钱)或环境杂物.物品必须唯一！若物品已在表中,无论它流转到哪里,都必须updateRow更新其[持有者]和[当前位置],严禁新建一行！
- 约定:仅记录双方明确达成共识的严肃承诺或誓言.必须包含{{user}}的主动确认.严禁记录单方面的命令、胁迫、日常行程安排或临时口头指令.
- 关于我(表8):记录{{user}}主动透露或对话中自然流露的关于自己的信息.主键为[信息类型],同一类型只能有一条记录！七大类型:①基础信息(姓名/昵称/性别/年龄/生日/职业/居住地/星座),②个人特质(性格/技能/爱好),③偏好习惯(兴趣爱好/饮食偏好/沟通方式/禁忌事项),④人际关系(家人/朋友/宠物/其他关系),⑤重要时刻(重要经历/约定事项,需标注具体时间如"2024-03-15 第一次见面"),⑥目标规划(短期目标3个月内/长期目标1年以上,格式:"[类型:短期/长期]目标内容,截止时间,进展状态".如"[短期-2024-06-01]考驾照-已报名,科目一已过"),⑦强调信息({{user}}主动说"你要记住/必须记住/不能忘记"的内容).若该类型已存在,必须用updateRow追加更新,严禁重复建行！表格必须在记录事件发生后立即填写并标注[记录时间].

【表8填写详细规则】
⚠️关键原则:同一[信息类型]只能存在一条记录！新信息必须追加到现有记录中,严禁重复建行！

1️⃣基础信息(首次记录用insertRow,后续更新用updateRow):
-内容:姓名、昵称、性别、年龄、生日、职业、居住地、星座等.
-格式:分号分隔,如"姓名:张三;昵称:小三;性别:男;年龄:25;生日:1999-05-20;职业:程序员;居住地:北京;星座:金牛座"
-更新示例:当{{user}}说"我换工作了,现在在做设计师",需updateRow修改职业字段,保持其他信息不变.

2️⃣个人特质(性格、技能、爱好):
-内容:{{user}}的性格特点、特殊技能、兴趣爱好.
-格式:分号分隔,如"性格:外向开朗;技能:会弹吉他,擅长烹饪;爱好:摄影,旅行"
-触发条件:{{user}}说"我是个外向的人"、"我会弹吉他"、"我喜欢摄影"等.

3️⃣偏好习惯(兴趣爱好、饮食偏好、沟通方式、禁忌事项):
-内容:日常偏好、饮食习惯、沟通风格、禁忌事项.
-格式:分号分隔,如"兴趣:看悬疑小说;饮食:不吃香菜,喜欢甜食;沟通:喜欢直接表达;禁忌:对花生过敏"
-重点:禁忌事项必须明确记录！如"对XX过敏"、"不能吃XX"、"讨厌XX".

4️⃣人际关系(家人、朋友、宠物、其他关系):
-内容:{{user}}的家庭成员、朋友、宠物等社交关系.
-格式:分号分隔,如"家人:父母健在,有个妹妹叫小美;朋友:大学室友老王,同事小李;宠物:养了只金毛叫大黄"
-触发条件:{{user}}提到"我爸妈"、"我有个朋友"、"我养了只狗"等.

5️⃣重要时刻(重要经历、约定事项,需标注具体时间):
-内容:{{user}}生命中的重要事件、里程碑、特殊约定.
-格式:必须包含具体日期！如"2024-03-15 第一次见面;2022年换了工作从深圳搬到上海;2020-06-01 大学毕业"
-触发条件:{{user}}说"我们第一次见面是XX时候"、"我XX年做了XX"、"记得XX时候我们约定了XX".
-⚠️注意:必须标注具体时间(年-月-日),若{{user}}未明说具体日期,根据对话上下文推断并记录当前对话时间.

6️⃣目标规划(短期目标3个月内/长期目标1年以上):
-内容:{{user}}的短期和长期目标,包含进展状态.
-格式:"[短期-截止日期]目标内容-当前进展;[长期-截止日期]目标内容-当前进展"
-示例:"[短期-2024-06-01]考驾照-已报名,科目一已过;[长期-2029年前]买房-正在存首付,已存20万"
-触发条件:{{user}}说"我打算XX"、"我的目标是XX"、"我正在准备XX".
-更新规则:当目标有新进展时,必须updateRow更新进展状态,如"科目一已过→科目二已过".

7️⃣强调信息({{user}}主动说"你要记住/必须记住/不能忘记"的内容):
-内容:{{user}}特别强调、反复提醒、要求记住的信息.
-格式:"{{user}}强调:具体内容"
-示例:"{{user}}强调:一定要记住他对花生过敏,绝不能吃花生及其制品"
-触发条件:{{user}}明确说"记住"、"别忘了"、"一定要"、"千万不要忘记"等强调性词汇.

⚠️操作规范:
-首次记录某类型信息:使用insertRow(8, {0: "信息类型", 1: "记录时间YYYY-MM-DD HH:mm", 2: "具体内容"})
-追加/更新已有类型:使用updateRow(8, 行索引, {1: "最后更新时间", 2: "更新后的完整内容"})
-严禁对同一类型使用insertRow重复建行！必须先检查该类型是否已存在,若存在则updateRow在原数据基础上追加或编辑！
-[记录时间]只记录该行的最后更新时间,格式:YYYY-MM-DD HH:mm(如"2024-03-15 14:20").
